...
public sealed partial class ServerGameTicker
{
    ...
    private void UpdateRoundFlow(float frameTime)
    {
        if (RunLevel == GameRunLevel.InRound)
        {
            ...
            var emptyDelay = Cfg.GetCVar(CCVarsStation7.EmptyServerRoundEndDelay);
            if (emptyDelay > 0f)
            {
                if (_playerManager.PlayerCount == 0)
                {
                    ...
                    {
                        _emptyServerRoundEndTime = Timing.CurTime + TimeSpan.FromSeconds(emptyDelay);
                        Log.Info($"Server is empty during round. Auto-ending in {emptyDelay}s if no players rejoin.");
                    }
                    else if (Timing.CurTime >= _emptyServerRoundEndTime.Value)
                    {
                        Log.Info("Server remained empty for the configured delay. Auto-ending round.");
                ...
                else
                {
                    if (_emptyServerRoundEndTime != null)
                    {
                        Log.Info("Player rejoined during empty-server countdown. Cancelling auto-end.");
                        _emptyServerRoundEndTime = null;
                    }
                }
            }
